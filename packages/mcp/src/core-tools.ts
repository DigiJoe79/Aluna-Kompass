...
export const coreMcpTools: McpToolDefinition[] = [
  ...
  t({ name: 'media_list', description: 'List media assets with size, type, folder and where each is used. Optional folder filter (omitted = all, null = root). Requires media.upload.', inputSchema: z.object({ folder: z.string().nullable().optional() }), handler: (deps, ctx, { folder }) => listMediaAssets(deps, ctx, folder), service: listMediaAssets }),
  ...
  t({ name: 'modules_list', description: 'List installed modules and whether they are enabled.', inputSchema: z.object({}), handler: async (deps) => ok(listModules(deps)), service: listModules }),
  ...
  t({ name: 'themes_list', description: 'List themes and the active theme key.', inputSchema: z.object({}), handler: async (deps) => ok(listThemes(deps)), service: listThemes }),

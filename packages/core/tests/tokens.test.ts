...
describe('api tokens', () => {
  ...
  /**
   * Die Oberfläche zeigte "zuletzt genutzt vor X Tagen" früher per
   * Date.now() im Browser gegen lastUsedAt — bei falsch gehender Client-Uhr
   * ein falscher Wert. Jetzt rechnet der Server mit derselben Uhr wie
   * lastUsedAt, die Anzeige übernimmt nur noch daysSinceLastUsed.
   */
  it('counts days since last use with the server clock, not the caller\'s', async () => {
    const deps = createTestDeps();
    const ctx = ctxWith([], insertUser(deps, {}));
    const { token, record } = unwrap(await createApiToken(deps, ctx, { name: 'Nie genutzt' }));
    expect(record.daysSinceLastUsed).toBeNull();

    resolveApiToken(deps, token, meta);
    deps.clock.advance(5 * 86_400_000);
    const [afterFiveDays] = unwrap(await listApiTokens(deps, ctx));
    expect(afterFiveDays!.daysSinceLastUsed).toBe(5);
  });
});

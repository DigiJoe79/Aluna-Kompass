...
export interface MediaUsage {
  id: string;
  filename: string;
  /** Labels der Fundstellen außerhalb des Datensatzes, der gerade gelöscht werden soll. */
  usedElsewhere: string[];
}

/**
 * Wer ein Asset sonst noch verwendet — für die Löschvorschau eines Datensatzes.
 * `self` ist der Datensatz selbst; seine eigenen Fundstellen zählen nicht.
 * Unbekannte IDs fallen weg, doppelte zählen einmal.
 */
...
export interface MediaCleanup {
  deletedMedia: string[];
  keptMedia: { id: string; filename: string; usedBy: string[] }[];
}

/**
 * Räumt die Medien eines gerade gelöschten Datensatzes ab. Jede Löschung läuft
 * über `deleteMediaAsset` — eigene Referenzprüfung, eigener Protokolleintrag,
 * wie von Hand. Was noch verwendet wird, bleibt und wird genannt.
 *
 * Bewusst keine gemeinsame Transaktion mit dem Datensatz: `deleteMediaAsset`
 * löscht nach seinem Commit Dateien. Bricht die Schleife ab, liegen die
 * restlichen Assets unreferenziert in der Mediathek und sind dort löschbar.
 */
export async function deleteUnreferencedMedia(deps: Deps, ctx: CallContext, assetIds: readonly string[]): Promise<Result<MediaCleanup>> {
  const denied = requirePermission(ctx, 'media.upload');
  if (denied) return denied;
  const cleanup: MediaCleanup = { deletedMedia: [], keptMedia: [] };
  for (const id of new Set(assetIds)) {
    const record = deps.db.select().from(mediaAssets).where(eq(mediaAssets.id, id)).get();
    if (!record) continue;
    const result = await deleteMediaAsset(deps, ctx, { id });
    if (result.ok) {
      cleanup.deletedMedia.push(id);
    } else if (result.error.type === 'conflict' && result.error.code === 'mediaAssetInUse') {
      cleanup.keptMedia.push({ id, filename: record.filename, usedBy: findMediaReferences(deps, id).map((r) => r.label) });
    } else {
      return result;
    }
  }

...
export interface ApiTokenSummary {
  ...
  /** Serverseitig berechnet, damit eine falsch gehende Client-Uhr die Anzeige nicht verfaelscht. */
...
const toSummary = (row: typeof apiTokens.$inferSelect, now: Date): ApiTokenSummary => ({
  id: row.id,
  name: row.name,
  prefix: row.prefix,
  ...
  daysSinceLastUsed: daysSince(row.lastUsedAt, now),
});
...
export async function revokeApiToken(deps: Deps, ctx: CallContext, input: unknown): Promise<Result<ApiTokenSummary>> {
  ...
  if (row.revokedAt) return ok(toSummary(row, deps.clock.now()));
  return deps.db.transaction((tx) => {
...
export async function listApiTokens(deps: Deps, ctx: CallContext): Promise<Result<ApiTokenSummary[]>> {
  ...
  const now = deps.clock.now();
  return ok(rows.map((row) => toSummary(row, now)));
}

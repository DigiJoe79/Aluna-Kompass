 ...
 * rechnet (Prinzip 2).
 */
export type RetentionClass = 'permanent' | 'statutory10Y' | 'statutory8Y' | 'statutory6Y' | 'consent';

export const RETENTION_CLASSES: readonly RetentionClass[] = ['permanent', 'statutory10Y', 'statutory8Y', 'statutory6Y', 'consent'];

/**
 * Vorgabelängen in Monaten.
 * - `statutory10Y`: Belege, Zuwendungsbestätigungen, Jahresabschluss (§ 147 AO).
 * - `statutory8Y`: Buchungsbelege (§ 147 Abs. 3 AO in der Fassung des BEG IV).

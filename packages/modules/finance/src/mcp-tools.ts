...
const runStartMcpSchema = z.object({ ...runArgsMcp, preNoticeReason: z.string().optional() });
...
export const FINANCE_MCP_TOOLS: readonly McpToolDefinition[] = [
  ...
  // F6b — Serienlauf und Spendenbuch.
  t({ name: 'finance_confirmation_run_preview', description: 'Preview the yearly run of donation confirmations: per contact one collective confirmation for money and fees, a separate one for expense waivers (always with a signature field) and one confirmation per in-kind line; grouped ready, needsSignature, addressMissing and blocked, with the number range and what blocks the whole run (no notice, association address). Anonymous lines, lines in a valid confirmation, excluded contacts and contacts below minCents (default finance.batchMinimumCents) stay out; followUpOfRunId only names the earlier run. Changes nothing. Requires finance.read.', inputSchema: runPreviewMcpSchema, handler: (deps, ctx, args) => previewConfirmationRun(deps, ctx, args), service: previewConfirmationRun }),
  t({ name: 'finance_confirmation_run_start', description: 'Start a run: snapshots the items of the preview as of today (the issue date of all its confirmations); ready and needsSignature items become pending, the others are skipped with their error code. preNoticeReason lets items with donations before the oldest notice run. Issues nothing yet - call finance_confirmation_run_continue. Human only: refused over MCP unless the association has set finance.mcpHumanOnlyAllowed at the screen. Requires finance.donationsIssue.', inputSchema: runStartMcpSchema, handler: (deps, ctx, args) => startConfirmationRun(deps, ctx, args), service: startConfirmationRun }),
  ...
  t({ name: 'finance_donation_reconciliation', description: 'Reconcile the donations of a year against valid confirmations: the difference split by reason (belowMinimum, addressMissing, inKindUndescribed, expenseWaiverUnconfirmed, anonymous, other) with count, cents and a link each; confirmations above what is left after returns show as toCorrect. Changes nothing. Requires finance.read.', inputSchema: donationReconciliationMcpSchema, handler: (deps, ctx, args) => getDonationReconciliation(deps, ctx, args), service: getDonationReconciliation }),
];

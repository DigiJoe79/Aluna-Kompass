...
export const projectDeleteSchema = z.object({ id: z.string().min(1), deleteOrphanedMedia: z.boolean().default(false) });

const assetIdsOf = (p: ProjectRecord): string[] => (p.imageAssetId ? [p.imageAssetId] : []);

/** Die eine Stelle, die über die Löschbarkeit eines Projekts entscheidet — für den Dienst und den Dialog. */
const deletionPreviewOf = (deps: Deps, p: ProjectRecord): DeletionPreview => buildDeletionPreview(deps, { entityType: 'project', id: p.id, isPublished: p.isPublished, assetIds: assetIdsOf(p) });

export async function projectDeletionPreview(deps: Deps, ctx: CallContext, id: string): Promise<Result<DeletionPreview>> {
  const denied = requirePermission(ctx, 'projects.view');
  if (denied) return denied;
  const project = load(deps.db, id);
  return project ? ok(deletionPreviewOf(deps, project)) : notFound('project', id);
}

/**
 * Löscht ein Projekt. Ein Projekt ist Webseiteninhalt; rechenschaftsrelevant
 * sind die Vorgänge, die daran hängen — Buchungen, Bescheide, Belege. Sie
 * melden sich als Halter oder Verweis und verhindern die Löschung. Zwei
 * Stufen: Veröffentlichtes wird erst zurückgezogen.
 */
export async function deleteProject(deps: Deps, ctx: CallContext, input: unknown): Promise<Result<MediaCleanup>> {
  const denied = requirePermission(ctx, 'projects.manage');
  if (denied) return denied;
  const parsed = validate(deps, projectDeleteSchema, input);
  if (!parsed.ok) return parsed;

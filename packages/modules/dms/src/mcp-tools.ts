...
import { previewReclassification, receiveDocument, receiveSchema, reclassifyDocument, reclassifyPreviewSchema, reclassifySchema } from './incoming';
...
export const DMS_MCP_TOOLS: McpToolDefinition[] = [
  // Lesen
  t({ name: 'dms_list', description: 'List and search documents in the file. Filters by direction, type, folder, phase, linked entity, relatedTo (another document), unsent (outgoing, filed, no dispatch note) and withOpenFollowUp; orderBy sorts by a column. The text filter searches subject, number and the recognised full text and returns the passage with its page for every full-text hit; terms shorter than three characters do not reach the full text. Requires dms.view or the permission of a protection area; shows only documents of types you may read.', inputSchema: documentListSchema, handler: (deps, ctx, args) => listDocuments(deps, ctx, args), service: listDocuments }),
  t({ name: 'dms_get', description: 'Read one document: metadata, links to contacts/animals/projects, relations to other documents, dispatch note, notes and follow-ups. Never the file itself — use dms_text for its content. Requires dms.view or the permission of a protection area; shows only documents of types you may read.', inputSchema: idSchema, handler: (deps, ctx, { id }) => getDocumentRecord(deps, ctx, id), service: getDocumentRecord }),
  t({ name: 'dms_text', description: 'Read the recognised full text of a document, page by page, or the state of recognition and why there is none yet. Requires dms.view or the permission of a protection area; shows only documents of types you may read.', inputSchema: documentTextSchema, handler: (deps, ctx, args) => getDocumentText(deps, ctx, args), service: getDocumentText }),
  t({ name: 'dms_types', description: 'List the document types with their number prefix and retention class; selectable: true lists only the types a person may choose. Requires dms.view or the permission of a protection area; shows only documents of types you may read.', inputSchema: documentTypeListSchema, handler: (deps, ctx, args) => listDocumentTypes(deps, ctx, args), service: listDocumentTypes }),
  t({ name: 'dms_folders', description: 'List the folders of the subject tree. Requires dms.view or the permission of a protection area; shows only documents of types you may read.', inputSchema: z.object({}), handler: (deps, ctx) => listDocumentFolders(deps, ctx), service: listDocumentFolders }),
  ...
  t({ name: 'dms_reclassify', description: 'Change type, subject or document date of a filed incoming document; the file never changes. A new type draws a new number from its prefix (same filing year), the old number stays on the document as formerNumbers and remains searchable; retention follows the new type and date. Outgoing and voided documents are refused. Pass expectedVersion (updatedAt you last read) to be rejected with staleVersion instead of overwriting a change made in between. Requires dms.create. Audited.', inputSchema: reclassifySchema, handler: (deps, ctx, args) => reclassifyDocument(deps, ctx, args), service: reclassifyDocument }),
  t({ name: 'dms_preview_reclassification', description: 'Show what dms_reclassify would do: the next number (null if the type stays, or if you may not read the target type — then numberHidden is true) and the retention before and after. Reads only. Requires dms.view or the permission of a protection area; shows only documents of types you may read.', inputSchema: reclassifyPreviewSchema, handler: (deps, ctx, args) => previewReclassification(deps, ctx, args), service: previewReclassification }),

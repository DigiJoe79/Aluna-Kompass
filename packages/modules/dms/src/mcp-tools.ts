...
export const DMS_MCP_TOOLS: McpToolDefinition[] = [
  ...
  t({ name: 'dms_types', description: 'List the document types with their number prefix and retention class; selectable: true lists only the types a person may choose. Requires dms.view.', inputSchema: documentTypeListSchema, handler: (deps, ctx, args) => listDocumentTypes(deps, ctx, args), service: listDocumentTypes }),

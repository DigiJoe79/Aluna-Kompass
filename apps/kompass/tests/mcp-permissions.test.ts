...
const OHNE_RECHT = new Set([
  'modules_list',
  'themes_list',
  'translations_list_gaps',
  // Die eigene Startseite (Spec 2026-09-17): Anordnung lesen, zuruecksetzen
  // und Inhalte lesen verlangen nur eine Sitzung, wie der Passwortwechsel.
  // Ohne Rechte kommt eine leere Kachelliste — jede Kachel traegt ihr Recht
  // selbst, der Test unten haelt das fest. `dashboard_set_layout` steht nicht
  // hier: mit leeren Argumenten scheitert es an der Validierung, und mit einer
  // Kachel ohne Recht an `forbiddenTile`.
  'dashboard_tiles',
...
describe('reading tools check permissions too', () => {
  ...
  it('offers a user without permissions no tile at all', async () => {
    const tiles = await werkzeug('dashboard_tiles').handler(deps, ohneRechte, {} as never);
    expect(tiles).toMatchObject({ ok: true, value: [] });
    const read = await werkzeug('dashboard_read').handler(deps, ohneRechte, {} as never);
    expect(read).toMatchObject({ ok: true, value: [] });
    const set = await werkzeug('dashboard_set_layout').handler(deps, ohneRechte, { tiles: [{ module: 'core', key: 'retention' }] } as never);

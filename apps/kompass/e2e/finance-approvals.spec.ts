...
import { backToAdmin, linkOwnContact, mcpClient, PHONE, rejectInQueue, submitClaim, switchToJonas } from './expense-helpers';
...
test.describe('finance approvals (D3)', () => {
  ...
  test('die Warteschlange zeigt die älteste zuerst und nie den eigenen Antrag; der direkte Aufruf zeigt den Sperrzustand', async ({ page, baseURL }) => {
    ...
    // Anna hat beide selbst angelegt: nichts in ihrer Schlange, der Link führt in den Sperrzustand.
    await page.goto('/finance/approvals');
    await expect(queue(page)).toContainText('Nichts wartet auf Ihre Freigabe.');
    ...
    await expect(items.nth(0)).toContainText(older.number!);
    await expect(items.nth(1)).toContainText(newer.number!);
    await expect(items.nth(0)).toContainText('Auslage');
    await expect(items.nth(0)).toContainText('Tomas Leitner');
    await expect(items.nth(0)).toContainText('12,50 €');
    await expect(items.nth(0)).toContainText(/seit \d{2}\.\d{2}\./);
    // Ohne Auswahl steht die älteste rechts.
    ...
    await expect(page).toHaveURL(`/finance/approvals?claim=${newer.id}`);
    await expect(detail(page).getByTestId('approval-number')).toHaveText(newer.number!);
  ...
  test('Verzicht: die vier Prüfungen als Checkliste; ohne Verzichtserklärung keine Freigabe; danach steht die Aufwandsspende als Buchung und ist bescheinigbar', async ({ page, baseURL }) => {
    ...
    await switchToJonas(page);
    await page.goto(`/finance/approvals?claim=${claim.id}`);
    await expect(queue(page).getByTestId('approval-queue-item').first()).toContainText('Verzicht');
  ...
  test('Freigeben auf 390 px mit klebender Fußleiste', async ({ page, baseURL }) => {
    ...
    await queue(page).getByTestId('approval-queue-item').first().click();
  ...
  test('die Kacheln zeigen „Wartet auf Ihre Freigabe“ und „Ihre Auslagen“ ohne fremde Namen', async ({ page, baseURL }) => {
    ...
    // Der eigene Antrag wartet nicht auf Annas Freigabe.
    await expect(page.getByTestId('dashboard-tile-finance-approvalsPending')).toContainText('Nichts wartet auf Ihre Freigabe.');

    await switchToJonas(page);
    await page.setViewportSize(PHONE);
    // Jonas hat eine gespeicherte Anordnung aus dem Seed — die Vorgabe bringt die neue Kachel.

...
/** Antwort von /site/job — die Dauer kommt fertig gerechnet vom Server, s.u. */
 ...
 *
 * Die Dauer rechnet der Server aus (`elapsedMs`), nicht diese Komponente: Eine
 * falsch gehende Uhr im Browser zeigte sonst ab dem ersten Tick einen falschen
 * Stand, egal wie kurz der Lauf tatsächlich schon geht.
 */
export function JobStatus({ onFinished }: { onFinished?: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    let previous: RunningJob | null = null;
    const tick = async () => {
      const current = await fetch('/site/job', { cache: 'no-store' })
        .then((r) => (r.ok ? (r.json() as Promise<RunningJob | null>) : null))
        .catch(() => null);
      if (!active) return;

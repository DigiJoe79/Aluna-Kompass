...
export function CreateDocumentDialog({ templates }: { templates: { key: string; enabled: boolean; reason?: string }[] }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[680px] max-w-[calc(100vw-2rem)] bg-surface shadow-md">
        <form action={action} className="flex flex-col gap-4" aria-busy={pending}>
          ...
          <FormField id="body" label={t('body')} hint={t('bodyHint')} error={errors.body}>
            <Textarea id="body" name="body" rows={8} maxLength={20000} value={body} onChange={(e) => setBody(e.target.value)} className="font-mono text-[13px]" />
          </FormField>
          <div>
            <p className="mb-1 text-[12px] font-semibold text-ink-2">{t('preview')}</p>

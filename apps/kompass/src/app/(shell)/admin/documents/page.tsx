import { hasPermission, listDocumentBases, listDocuments, listUsers, readSetting, requirePermission } from '@kompass/core';
...
export default async function DocumentsPage(props: { searchParams: Promise<{ selected?: string }> }) {
  ...
  const titleOf = (snap: unknown) => {
    const s = snap as { slots?: { title?: string }; input?: { title?: string } };
    return String(s.slots?.title ?? s.input?.title ?? '');
  };
  const rows = result.value.documents.map((d) => ({ ...d, createdByName: nameOf(d.createdByUserId), title: titleOf(d.inputSnapshot) || d.number }));
  const canCreate = hasPermission(ctx, 'documents.create');
  const registered = [...deps.registry.documentTemplates.values()];
  const templates = registered.map((tpl) => ({ key: tpl.key, enabled: !tpl.permission || hasPermission(ctx, tpl.permission), reason: tpl.permission && !hasPermission(ctx, tpl.permission) ? t('create.missingPermission', { permission: tpl.permission }) : undefined }));

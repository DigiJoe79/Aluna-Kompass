...
- `pnpm e2e:image` — die E2E-Suite gegen ein gebautes `kompass-local`, ein Container `kompass-e2e-<Platz>` je Worker ab Port 3200, drei auch online (`E2E_IMAGE` für ein anderes Bild). Prüft die Verpackung: gebündelter Code, `/data`-Volume, mitgeliefertes Template, Modulauflösung.

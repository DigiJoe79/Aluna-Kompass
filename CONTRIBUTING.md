...
**Pull Requests öffnen nur Mitarbeitende.** Das Repo ist so eingestellt, weil
es im Moment eine Person betreut. Wer etwas beitragen möchte, beginnt mit einem
Issue: Wir klären dort, ob es passt, und du wirst für die Arbeit daran als
Mitarbeitende:r eingetragen.
...
Vor dem Code steht eine kurze Spec: was gebaut wird und warum, welche Säule
aus [`docs/nordstern.md`](docs/nordstern.md) sie betrifft. Für einen Beitrag
von außen ist das Issue der Ort dafür — die Specs des Projekts selbst liegen
nicht im öffentlichen Repo. Das klingt nach Overhead, spart aber die
Diskussion, die sonst im Pull Request stattfindet, wenn die Arbeit schon getan
ist.

...
2. **Anmeldung:** IONOS-Webhosting bietet keine SSH-Schlüssel, deshalb Passwort-Login. Das Webspace-Passwort in eine Datei `site.pw` schreiben (nur die Zeile mit dem Passwort, kein Zeilenumbruch nötig) und nach `/share/Container/kompass-test/` **und** `/share/Container/kompass-prod/` legen, Rechte 600, Besitzer UID 1000 (`node`):
   ...
   Kompass ruft `sshpass -f /data/site.pw rsync -az --no-owner --no-group --delete --checksum …` auf; `--no-owner`/`--no-group`, weil `-a` sonst versucht, Besitzer und Gruppe am Ziel zu setzen — das darf nur root, und auf einem Webspace bricht der Lauf mit „chgrp … Operation not permitted" ab. das Passwort steht damit nie in der Prozessliste und nicht in `docker inspect`. Auf einem Hoster mit Schlüsselanmeldung stattdessen `SITE_DEPLOY_KEY_FILE` setzen — der Code beherrscht beides.

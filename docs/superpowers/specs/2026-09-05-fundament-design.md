...
Verbindlich aus dem Design übernommen: Token-Schema und Default-Werte (`1a`), Shell-Maße (Sidebar 248/56 px, Topbar 56 px, Drawer unter 1180 px — Sidebar-Maße abgelöst am 2026-09-14, siehe Navigations-Spec; Topbar und Drawer-Grenze gelten weiter), Umgebungsbalken-Farben (Test `#1A1A1A`/`#F2C200`, Dev `#B3261E`/`#FFFFFF` — die einzigen nicht themebaren Farbwerte), Rechte-Matrix mit sichtbaren Permission-Keys, Feld-Diff im Protokoll, Einmal-Dialoge ohne Schließen-× (Startpasswort, API-Token), Import-Bestätigung durch Eintippen des Umgebungsnamens, Fehlerseiten innerhalb der Shell, drei Lademuster (Balken, Schrittliste ohne Abbrechen, Skelett).
